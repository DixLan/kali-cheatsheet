...
```shell
DixLan@htb[/htb]$ find /etc/ -name shadow
```
...
```shell
DixLan@htb[/htb]$ find /etc/ -name shadow 2>/dev/null
```
...
```shell
DixLan@htb[/htb]$ find /etc/ -name shadow 2>/dev/null > results.txt
```
...
```shell
DixLan@htb[/htb]$ find /etc/ -name shadow 2> stderr.txt 1> stdout.txt
```
...
```shell
DixLan@htb[/htb]$ find /etc/ -name passwd >> stdout.txt 2>/dev/null
```
...
```shell
DixLan@htb[/htb]$ cat << EOF > stream.txt
```
...
```shell
DixLan@htb[/htb]$ find /etc/ -name *.conf 2>/dev/null | grep systemd
```
...
```shell
DixLan@htb[/htb]$ find /etc/ -name *.conf 2>/dev/null | grep systemd | wc -l
```
